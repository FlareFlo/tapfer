...
pub(crate) mod error_compat {
	...
	impl<T, E> InternalServerErrorExt<T> for Result<T, E> {
		fn ise(self) -> ApiResult<T> {
			self.map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Html("Internal Server Error".to_owned())))
		}
	}

	impl<T> InternalServerErrorExt<T> for Option<T> {
		fn ise(self) -> ApiResult<T> {
			self.ok_or((StatusCode::INTERNAL_SERVER_ERROR, Html("Internal Server Error".to_owned())))
		}
	}
}
